...
# Encrypt a .env file in Cloudflare Workers
...
`}<span className="logger-success">◈ encrypted (.env.txt)</span></code>

<code title="with Armor ⛨" code="npx dotenvx armor login">{`$ npx dotenvx armor login
$ npx dotenvx encrypt -f .env.txt
`}<span className="logger-success">◈ encrypted (.env.txt) · armored ⛨</span></code>
