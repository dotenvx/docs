...
<CodeGroup>
<code title="Dotenvx" code={`dotenvx run -- sh -c 'echo Hello $HELLO'`}>{`$ dotenvx run -- sh -c 'echo Hello $HELLO'
`}<span className="logger-success">⟐ injected env (14) from .env</span>{`
Hello Dotenvx`}</code>
</CodeGroup>
