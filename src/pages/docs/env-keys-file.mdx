...
The .env file format will still be at the center of security. But we have added a new extension. Its is optional, but we highly recommend it for production:

* [.env.vault](/docs/security/env-vault) extension

In the next set of security docs you can read about how it works alongside your .env files to significantly improve security. This is the next step forward in application secrets security, and like the original .env file format we have worked hard to minimize complexity in order to increase security.
