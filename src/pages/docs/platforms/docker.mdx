...
Use `dotenvx` to encrypt your `.env.production` file.

```sh
dotenvx set HELLO production -f .env.production --encrypt
```

This generates a `.env.keys` file containing your production decryption key.
...
Your `.env.production` file is now encrypted and contains the production encryption key.
...
<Note>
  You SHOULD commit your `.env.production` to code. It's now encrypted, safe, and recommended to do so.
</Note>
