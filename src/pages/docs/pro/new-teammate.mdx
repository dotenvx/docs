...
<Note>
  The private key NEVER leaves the user's machine and sits encrypted at rest. Pro's service NVER has access to it. It is accessible solely by the cli during local decryption processes.
</Note>
...
The organization private key is an implementation detail that dotenvx pro obfuscates away for you. But if you do want to it, you can with `dotenvx pro settings orgprivatekey --unmask`.
