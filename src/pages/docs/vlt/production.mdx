export const description =
  'Deploy with `dotenvx-vlt`'

##### [Vlt](/docs/vlt)
# Production

Ready to deploy to production? Here's how with  `dotenvx` and `dotenvx-vlt`. {{ className: 'lead' }}

Deploying to production with `dotenvx-vlt` is the same as deploying with `dotenvx`.
